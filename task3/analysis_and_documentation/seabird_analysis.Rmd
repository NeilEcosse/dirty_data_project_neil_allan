...
read_csv(here("clean_data/bird_data_codes_clean.csv"))

ship_data_codes_clean <-
read_csv(here("clean_data/ship_data_codes_clean.csv"))
```
...
```{r}
names(ship_data_by_record_id_clean)
```
...
```{r}
ship_data_by_record_id_clean %>% 
  mutate(year = substring(ship_data_by_record_id_clean$date,1,4))
```
